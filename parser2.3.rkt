...
(define scanner
         ...
         [(:or "<" ">" ">=" "<>" "<=")(cons `(BOOLOP ,(string->symbol lexeme),n)(scanner input-port))] ;test
...
(define (bool-op token-list)
  (if(equal? (car(first token-list)) 'BOOLOP)(success (rest token-list))((failure (cdr(first token-list))))))
